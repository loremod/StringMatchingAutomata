...
/*	Ho fatto questa classe per poter gestire l'utilizzo dei campioni di testo
	...
	In questo modo, quando dovr� comparare gli algoritmi di string matching(specialmente per quanto riguarda 
	i tempi di esecuzione) potr� importare a run-time il testo che pi� preferisco, potendomi anche basare sulla dimensione
	dei vari file(espressa con gli asterischi, richiamando il metodo printStarryInfo()).
	Cos� facendo posso fare dei test utilizzando un testo molto grande e che contiene un alfabeto di caratteri alfanumerici vasto(interessante
...
class samplesManager{
	...
	public:
		...
		static bool directoryExists(char _dir[]){
			DIR *dr = opendir(_dir);
			if (dr == NULL)  // ritorna false se non pu� aprire la directory
				return false;
			return true;
		}
		string* getFileNames(){
